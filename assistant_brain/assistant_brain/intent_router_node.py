"""assistant_brain 의도 라우터 노드.

역할:
- 전사 텍스트를 command/query/chat 카테고리로 분류
- 의도 이벤트를 `/assistant/intent`로 발행
- 카테고리별 처리 결과를 `/assistant/speak`, `/assistant/status_text`로 반영
"""
...
class IntentRouterNode(Node):
    """전사 텍스트를 의도로 분류하고 command / query / chat 으로 라우팅 기능.
    ...
    def _publish_intent(self, c: IntentClassification) -> None:
        ...
        msg.intent_category = c.intent_category
    ...
    def _handle_command(self, c: IntentClassification) -> None:
        ...
        if intent_name == 'stop':
            # stop은 safety_gate가 emergency_stop을 발행하므로
            # 이 노드에서는 추가 액션 없이 intent 전달만 유지한다.
            return
